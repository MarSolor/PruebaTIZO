describe("Test Suite - Inicio de Sesión A", () => {
    ...
    function iniciarRecuperacionContrasena(telefono) {
        cy.get(':nth-child(4) > .text-sm').click().wait(100);
        cy.get('.text-sm').type(telefono).wait(100);
        cy.get(':nth-child(4) > .w-full').click().wait(100);
    }

    // Función para establecer una nueva contraseña
    function establecerNuevaContrasena(codigoVerificacion, nuevaContrasena) {
        const { codigo1, codigo2, codigo3, codigo4 } = descomponerCodigo(codigoVerificacion);

        cy.get('.relative.mt-5 > #pass').type("01234").wait(100);
        cy.get(':nth-child(2) > #pass').type(nuevaContrasena).wait(100);
        cy.get('#pass1').type(nuevaContrasena).wait(100);
        cy.get('.flex-row > .w-full').click().wait(1000);
    }
